...
#@jit(nopython=False,forceobj=True,looplift=True,fastmath = True,parallel = True)
    ...
    def __getitem__(self, item):
                ...
                if calib_temp.cu == calib.cu and calib_temp.cv == calib.cv and calib_temp.fu == calib.fu and calib_temp.fv == calib.fv:
                    ...
                    if dst_W_temp == dst_W and dst_H_temp == dst_H:
                        ...
                        if len(objects_1) + len(objects_2) < self.max_objs: 
                            ...
                            # NOTE:alpha = 0.5
                            # img_blend = Image.blend(img, img_temp, alpha=np.clip(np.random.normal(loc=0.5, scale=0.1), 0, 1))
                            # img_blend = Image.blend(img, img_temp, alpha=np.random.beta(2,2))
        ...
        if self.split!='test':
            ...
            for i in range(object_num):
                ...
                #print(objects[i].cls_type,self.writelist)
                ...
                # generate the radius of gaussian heatmap # 左上,右下
                ...
                #size_2d[i] = 1. * w, 1. * h
                size_2d[i] = w/2 , h/2, w/2 , h/2 # 东南西北
                ...
                #print(objects[i].trucation,objects[i].occlusion)
            ...
            # if False:
                ...
                for i in range(object_num_temp):
                    ...
                    size_2d[i + object_num] = w/2 , h/2, w/2 , h/2 # 东南西北
        ...
        info = {'img_id': index,
                ...
                'get_item_time':time.time()-start_time,
                'count':self.count}
