...
class OpenLane_dataset(Dataset):
    ...
    def __init__(self, image_path, 
        ...
        self.anchors = build_anchor([3,4,5,6,7,8,10,15,30,45,60,75,90], [6,2,-2,-6]).T / self.point_range # [3,0]
        ...
        #     for n in self.label_stash:
        #         tasks.append(executor.submit(self.load_json, n))
        #     task_pbar = tqdm(futures.as_completed(tasks), total=len(tasks))
        #     for index,task in enumerate(task_pbar):
        #         results.append(task.result())
        #         task_pbar.set_description(f'load json to RAM iter>>>{index}')
        #         task_pbar.update()
    ...
    def build_pkl(self):
        ...
        for index,label in tqdm(enumerate(self.label_stash),'build pkl'):
    ...
    def __getitem__(self, index):
        ...
        for index,anchor_point in enumerate(self.anchors.T):
            ...
            if index % 4 == 0:  # left_left
                if lane_left_left.numel() != 0:
                    kdt   = AnnoyIndex(3, 'euclidean')
                    for index,point in enumerate(lane_left_left.T):
                        kdt.add_item(index+1,point)
                    kdt.add_item(0,anchor_point)
                    kdt.build(10)
                    new_find = kdt.get_nns_by_item(0,2)[1] - 1
                    new_find = lane_left_left.T[new_find].unsqueeze(1)
                near_left_left = torch.cat((near_left_left,new_find),dim=1)
                dis_left_left = torch.cat((dis_left_left,anchor_point.unsqueeze(1) - new_find),dim=1)
                #dis_left_left = torch.cat((dis_left_left,new_find),dim=1)
            elif index % 4 == 1: # left
                if lane_left.numel() != 0:
                    kdt   = AnnoyIndex(3, 'euclidean')
                    for index,point in enumerate(lane_left.T):
                        kdt.add_item(index+1,point)
                    kdt.add_item(0,anchor_point)
                    kdt.build(10)
                    new_find = kdt.get_nns_by_item(0,2)[1] - 1
                    new_find = lane_left.T[new_find].unsqueeze(1)
                near_left = torch.cat((near_left,new_find),dim=1)
                dis_left = torch.cat((dis_left,anchor_point.unsqueeze(1) - new_find),dim=1)
                #dis_left = torch.cat((dis_left,new_find),dim=1)
            elif index % 4 == 2: # right
                if lane_right.numel() != 0:
                    kdt   = AnnoyIndex(3, 'euclidean')
                    for index,point in enumerate(lane_right.T):
                        kdt.add_item(index+1,point)
                    kdt.add_item(0,anchor_point)
                    kdt.build(10)
                    new_find = kdt.get_nns_by_item(0,2)[1] - 1
                    new_find = lane_right.T[new_find].unsqueeze(1)
                near_right = torch.cat((near_right,new_find),dim=1)
                dis_right = torch.cat((dis_right,anchor_point.unsqueeze(1) - new_find),dim=1)
                #dis_right = torch.cat((dis_right,new_find),dim=1)
            elif index % 4 == 3: # right_right
                if lane_right_right.numel() != 0:
                    kdt   = AnnoyIndex(3, 'euclidean')
                    for index,point in enumerate(lane_right_right.T):
                        kdt.add_item(index+1,point)
                    kdt.add_item(0,anchor_point)
                    kdt.build(10)
                    new_find = kdt.get_nns_by_item(0,2)[1] - 1
                    new_find = lane_right_right.T[new_find].unsqueeze(1)
                near_right_right = torch.cat((near_right_right,new_find),dim=1)
                dis_right_right = torch.cat((dis_right_right,anchor_point.unsqueeze(1) - new_find),dim=1)
                #dis_right_right = torch.cat((dis_right_right,new_find),dim=1)
...
def build_anchor(anchor_x,anchor_y,anchor_z=-2.1):
    points = []
    for x in anchor_x:
        for y in anchor_y:
...
if __name__ == "__main__":
    ...
    anchors = build_anchor([3,4,5,6,7,8,10,15,30,45,60,75,90], [6,2,-2,-6]).T / point_range.T
    ...
    for index,(image,target,info) in enumerate(test_dataload):
        ...
        #target_line = target_p
        print(target_line,target_p)
        
        target_line = target_line * torch.tensor([[90,6,2]]).T
        anchors = anchors * torch.tensor([[90,6,2]]).T
        #ouptput_line = ouptput_line * torch.tensor([[20,2,3]]).T
