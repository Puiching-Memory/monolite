...
if __name__ == '__main__':
    batch_size = 1
    box_num = 3
    input = torch.zeros(batch_size,box_num,7)
    #target = torch.Tensor(range(batch_size*21)).reshape(batch_size,3,7)
    target = torch.rand(batch_size*box_num*7).reshape(batch_size,box_num,7)